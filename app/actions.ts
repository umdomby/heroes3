...
const MARGIN = process.env.MARGIN || 0.05; // Маржа 10%
...
export async function placeBet(formData: { betId: number; userId: number; amount: number; player: PlayerChoice }) {
    try {
        ...
        const updatedBetData = {
            oddsBetPlayer1: oddsPlayer1 * process.env.CORRECT_ODDS || 0.85,
            oddsBetPlayer2: oddsPlayer2 * process.env.CORRECT_ODDS || 0.85,
            ...
            for (const participant of allParticipants) {
                ...
                if (participant.isWinner) {
                    ...
                    if (participant.isCovered === "CLOSED" && participant.profit === participant.overlap) {
                        ...
                        pointsToReturn = (participant.amount - (participant.overlap / participant.odds))/ process.env.LOSE_PENDING_BET || 2.5;
                    }
                }
